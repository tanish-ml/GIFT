...
export function resolveDownloadPath(platform, buildId) {
    return [
        ...
        `chrome-headless-shell-${folder(platform, buildId)}.zip`,
    ];
}
export function relativeExecutablePath(platform, buildId) {
    switch (platform) {
        case BrowserPlatform.MAC:
        case BrowserPlatform.MAC_ARM:
            return path.join('chrome-headless-shell-' + folder(platform, buildId), 'chrome-headless-shell');
        case BrowserPlatform.LINUX_ARM:
        case BrowserPlatform.LINUX:
            return path.join('chrome-headless-shell-' + folder(platform, buildId), 'chrome-headless-shell');
        case BrowserPlatform.WIN32:
        case BrowserPlatform.WIN64:
            return path.join('chrome-headless-shell-' + folder(platform, buildId), 'chrome-headless-shell.exe');
    }
}
